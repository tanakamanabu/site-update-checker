...
- **複数対象**: `config.targets[]` に `{ name, baseUrl, basicAuth, ...上書き }` を並べる。`src/target.js` の `resolveTarget(name)` が実行時引数から対象を選び、共通設定（concurrency / timeout / diffThreshold / viewport / excludePatterns / stayOnDomain / userAgent / disableAnimations / screenshotDelay / hideSelectors / injectCSS / blockResources / checkExternalLinks / linkCheckTimeout / linkConcurrency）とマージして返す。`reportDir` は `reports/<name>` に解決され、対象ごとにレポートが分離される。対象が複数あるのに名前未指定だとエラーで一覧を出す。
- `crawl.js`: 引数 `[before|after] [対象名]`。解決済み `baseUrl` を起点に BFS でクロール、`concurrency` 件ずつ並列。`excludePatterns`（正規表現）と `stayOnDomain` で対象を絞る。リンク切れはページ内で `fetch(HEAD)` を実行しステータス判定。結果は各フェーズの `results.json` に保存。`results.json` にはクロールの `startedAt`（開始時刻、初回書き込み前に一度確定）と `finishedAt`（=各書き込み時刻なので最後の書き込み＝終了時刻）を ISO(UTC) で記録する（`crawledAt` は `finishedAt` と同値で後方互換用に残置）。
- **リンク切れチェックの高速化（`crawl.js` の `checkLinkStatus`）**: ブラウザの `fetch` にはタイムアウトが無く、応答しない外部リンク（SNS共有・他サイト等）に当たると既定のネットワークタイムアウトまで固まる。これが「特定ページだけ極端に遅い→バッチ全体が止まり空白時間ができる」主因だった。対策として (1) `AbortController` で `linkCheckTimeout`（既定8s）の上限を付与、(2) 逐次 `for await` をやめ `Promise.all` で並列化（1ページの待ちを「合計」から「最遅1本」へ）、(3) 確認結果を URL 単位で `linkStatusCache`(Map) にメモ化（全ページ共通の外部リンクの重複チェックを排除）、(4) `javascript:`/`mailto:` 等の擬似リンクや `http://-/` 等の不正ホストは `util.js` の `isCheckableHttpLink` で確認対象外に。`checkExternalLinks`（既定 true）を false にすると外部ドメインのリンクチェック自体をスキップしてさらに高速化（内部リンクのみ確認）。
- **リンクチェックの並列度制御と 429（レート制限）対策（`crawl.js`）**: 従来は1ページ内の全リンクを `Promise.all` で一斉に叩き、さらにページ自体も `concurrency` 件並列だったため、瞬間同時リクエストが `concurrency × ページ内リンク数` まで膨らみ対象サーバーに 429（Too Many Requests）を出させていた。対策として (1) 全ページ通算の軽量リミッタ `createLimiter`（外部依存なし）で同時 fetch を `linkConcurrency`（既定6）件に頭打ち、(2) `linkStatusCache` は結果ではなく「確認中の Promise」を入れて同一 URL の同時確認を1回の fetch に集約（結果キャッシュだとレースで重複 fetch が走っていた）、(3) `checkLinkStatus` 内で 429 は `Retry-After`（秒、無ければ指数バックオフ、上限5s）を尊重して最大2回リトライ、(4) リトライしても 429 のリンクは**リンク切れ（死亡）に誤計上せず**、`results.json` の `rateLimited[]` と各ページ `outboundRateLimited[]` に分離記録。`diff.js` はこれを「レート制限で確認できなかったリンク（429・要確認）」セクション＋サマリーカードで別枠表示する（旧 json には無いので `?? []` フォールバック）。429 が多発するなら `linkConcurrency` を下げる（例 3〜4）。
- **スクショ安定化（`crawl.js` の `stabilizePage`）**: `goto` は `waitUntil:"networkidle"` だが、フェードイン演出は networkidle 後に再生されるため、そのまま撮ると before/after で別々の中間フレームを撮って誤検出になる。撮影直前に (1) CSS アニメーション/トランジションを `0s !important` で無効化して最終状態へ飛ばす、(2) ページ全体を段階スクロールして IntersectionObserver 系の遅延表示を発火させ先頭へ戻す、(3) ドキュメント高さが 3 回連続で変わらなくなるまで待つ（最大~2s）、(4) `screenshotDelay`(ms) だけ待つ（JS/rAF 駆動のフェードイン対策）。`disableAnimations`（既定 true）/ `screenshotDelay`（既定 0）で制御。演出自体の変化を見たい対象だけ `disableAnimations:false`。(3) の高さ安定待ちは、fullPage スクショで「測定した scrollHeight」と「実際に paint された高さ」が食い違って末尾が純黒で埋まる**末尾黒帯アーティファクト**を防ぐためのもの（直前のスクロールで遅延要素や 100vh/sticky が再計算され高さが揺れている最中に撮ると起きる）。
- **動的要素の非表示（`crawl.js` の `stabilizePage`）**: カルーセル・広告枠・ランダム表示バナー・時計など、before/after で中身が変わって誤差分を生む動的要素を撮影前にCSSで隠せる。`hideSelectors`（セレクタ配列）は各セレクタに `display:none !important` を当て、`injectCSS`（生CSS文字列）はそれで足りない上書き（visibility/位置/色固定など）をそのまま注入する。注入は `disableAnimations` のCSSの後・スクロール／高さ安定待ちの前に行い、要素を消した後の高さで fullPage スクショが安定するようにする。両方既定は無効（`[]` / `""`）。隠すとレイアウト高さが before/after 共通で変わるだけなので、変更したら before/after を両方撮り直すこと。`#wrapper` 配下の演出で誤検出が出る対象向け。
...
- `diff.js`: 引数 `[対象名]`。before/after を URL でマッチング。SS はサイズが違う場合は大きい方に白埋めリサイズしてから pixelmatch で比較。**本番チェックの自動化向けにヒットミスを避ける方針**: 1px でも差分があれば一覧に出し（diff 画像も出力）、`diffThreshold`（比率）を超えたものだけ「要確認」として強調表示する（`util.js` の `classifyVisualChange` が `none/minor/significant` を判定）。差分率は 0.00% に丸まる微小変更でも分かるよう実 px 数も併記。`diffThreshold` は「出す/出さない」ではなく「強調する/しない」の閾値。**納品用に self-contained 化**: `report.html` は `reportDir/report/` に出力し、レポートで実際に使う画像だけを `report/assets/` に集約コピーする（before/after は同名衝突を避け `before_`/`after_` を前置、diff は `diff_` のまま）。相対パスは `./assets/...`。`report/` フォルダごと渡せば before/after/diff の作業データ（全ページの生スクショ）を含めず分離できる。`assets/` は毎回作り直す。ビジュアル差分 / リンク切れ / ステータス変化 / 新規・削除ページを分類して単一 HTML に出力。レポート冒頭に before/after クロールの開始・終了日時とレポート作成日時を表示する（`results.json` の `startedAt`/`finishedAt` を `util.js` の `formatDateTime` で JST 表記に整形。旧 json 互換で `finishedAt` 不在時は `crawledAt` にフォールバック）。**末尾黒帯アーティファクトの自動補正**: before/after で高さが違い、高い方の余剰末尾領域が全幅・純黒の一様帯（`util.js` の `isUniformBlackBand` で判定）なら、fullPage スクショのアーティファクトとみなして差分から除外（`realNumDiff = numDiff - bandPx`）。分類・差分率表示はこの実差分で行うので黒帯で「要確認」に化けない。純黒帯だけのページはビジュアル差分から外れ、専用セクション「末尾黒帯アーティファクト（自動補正）」に一覧する（誤判定の確認用）。実差分も併存する場合はビジュアル差分側に「末尾黒帯補正」タグを付ける。
- `check.js`: `spawnSync` で after → diff を順に呼び、対象名引数をそのまま引き継ぐ（npm の `--` 経由だと両方に引数が渡らない問題を回避するため）。
- `util.js`: I/O・`process.exit`・グローバル設定参照を持たない純粋関数だけを置く（`urlToFilename` / `isNetworkUnreachable` / `isSameDomain` / `isExcluded` / `isCheckableHttpLink` / `isUniformBlackBand` / `escapeHtml` / `toPercent` / `formatDateTime` / `detectMissingScreenshot` / `classifyVisualChange`）。`urlToFilename` は元 URL 全体の短いハッシュ（sha1 先頭8桁）を末尾に必ず付け、可読部は小文字化する。これで大文字小文字を区別しないFS（Windows/macOS）でも別 URL 同士が同名化して上書きされる衝突を防ぐ（同一 URL は常に同名なので before/after は一致する）。crawl.js / diff.js は import 時にトップレベル await で処理を開始するためそのままではテストできないので、テストしたいロジックはここへ切り出す方針。
...
- カルーセル・広告・ランダム表示・時計など中身が毎回変わる動的要素で誤差分が出る場合は `hideSelectors`（例 `["#wrapper .carousel", "#ad"]`）で撮影前に隠す。細かい上書きが要るなら `injectCSS` に生CSSを書く。隠すと高さが変わるので before/after を両方撮り直すこと。
