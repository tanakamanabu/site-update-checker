...
- `util.js`: I/O・`process.exit`・グローバル設定参照を持たない純粋関数だけを置く（`urlToFilename` / `isNetworkUnreachable` / `isSameDomain` / `isExcluded` / `escapeHtml` / `toPercent` / `detectMissingScreenshot`）。crawl.js / diff.js は import 時にトップレベル await で処理を開始するためそのままではテストできないので、テストしたいロジックはここへ切り出す方針。
