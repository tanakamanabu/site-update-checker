...
- **特定ページだけ極端に遅い／定期的にクロールが止まるとき**は、応答しない外部リンクのリンク切れチェックが原因のことが多いです。`linkCheckTimeout`（既定8000ms）で1リンクあたりの待ち時間に上限が付きます。外部リンクのチェック自体が不要なら `checkExternalLinks: false` で内部リンクのみ確認し、大幅に高速化できます
